...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: vcc.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: vccd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizevccamount=<n> " + _("Keep N VCC anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. VCC is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. VCC is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("VCC version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "VCC server starting\n"); 
...
#ifdef ENABLE_WALLET
    if (mapArgs.count("-reservebalance")) // vcc: reserve balance amount
    {
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of VCC") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart VCC to complete") << "\n";
    ...
    nAnonymizeVCCAmount = GetArg("-anonymizevccamount", 0);
    if(nAnonymizeVCCAmount > 999999) nAnonymizeVCCAmount = 999999;
    if(nAnonymizeVCCAmount < 2) nAnonymizeVCCAmount = 2;
    ...
    LogPrintf("Anonymize VCC Amount %d\n", nAnonymizeVCCAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1VCC+1000 == (.1VCC+100)*10
       10VCC+10000 == (1VCC+1000)*10
    */
