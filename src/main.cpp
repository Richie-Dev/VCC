...
const string strMessageMagic = "VCC Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    if (nHeight == 1) {
        nSubsidy = 10000000 * COIN; // premine 10%
    } else if (nHeight >= 2 && nHeight < 1001) {
        nSubsidy = 0 * COIN;
    } else {
        nSubsidy = 0 * COIN;
    }
    LogPrintf("POW Block: %s\n", nHeight);
    LogPrintf("POW Reward: %s VCC\n", nSubsidy);
...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees)
{
    ...
    if (pindexBest->nHeight + 1 > 1 && pindexBest->nHeight + 1 < 9000000) {
        nSubsidy = 10 * COIN;
    }
    LogPrintf("POS Block: %s\n", pindexBest->nHeight + 1);
    LogPrintf("POS Reward: %s VCC\n", nSubsidy);
    return nSubsidy + nFees;
}
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue)
{
    ...
    LogPrintf("Masternode Block Value: %s\n", blockValue);
    LogPrintf("Masternode Reward:      %s VCC\n", ret);
...
bool AcceptToMemoryPool(CTxMemPool& pool, CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
    ...
    // vcc: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
bool CTransaction::ConnectInputs(CTxDB& txdb, MapPrevTx inputs, map<uint256, CTxIndex>& mapTestPool, const CDiskTxPos& posThisTx, const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags, bool fValidateSig)
{
    ...
    if (!IsCoinBase()) {
        ...
        for (unsigned int i = 0; i < vin.size(); i++) {
            ...
            // vcc: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // vcc: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake()) {
        // vcc: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    // vcc: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // vcc: compute stake entropy bit for stake modifier
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock)) {
        ...
        if (pfrom) {
            ...
            // vcc: getblocks may not obtain the ancestor block rejected
...
#ifdef ENABLE_WALLET
// vcccoin: attempt to generate suitable proof-of-stake
