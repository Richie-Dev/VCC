...
Value darksend(const Array& params, bool fHelp)
{
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "darksend <vccaddress> <amount>\n"
            "vccaddress, reset, or auto (AutoDenominate)"
    ...
    if (params.size() != 2)
        throw runtime_error(
            "darksend <vccaddress> <amount>\n"
            "vccaddress, denominate, or auto (AutoDenominate)"
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (fHelp  ||
        ...
        throw runtime_error(
                ...
                "  start        - Start masternode configured in vcc.conf\n"
                "  start-alias  - Start single masternode by assigned alias configured in masternode.conf\n"
                "  start-many   - Start all masternodes configured in masternode.conf\n"
                "  status       - Print masternode status information\n"
                "  stop         - Stop masternode configured in vcc.conf\n"
                ...
                "  vote-many    - Vote on a VCC initiative\n"
                "  vote         - Vote on a VCC initiative\n"
                );
    ...
    if (strCommand == "genkey")
    {
        ...
        return CVCCcoinSecret(secret).ToString();
    }
...
Available modes:
  ...
  votes           - Print all masternode votes for a VCC initiative (can be additionally filtered, partial match)
