...
namespace {
    class CVCCcoinAddressVisitor : public boost::static_visitor<bool> {
    private:
        CVCCcoinAddress *addr;
    public:
        CVCCcoinAddressVisitor(CVCCcoinAddress *addrIn) : addr(addrIn) { }
...
bool CVCCcoinAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CVCCcoinAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CVCCcoinAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CVCCcoinAddressVisitor(this), dest);
}

bool CVCCcoinAddress::IsValid() const {
...
CTxDestination CVCCcoinAddress::Get() const {
    if (!IsValid())
...
bool CVCCcoinAddress::GetKeyID(CKeyID &keyID) const {
...
bool CVCCcoinAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CVCCcoinSecret::SetKey(const CKey& vchSecret) {
...
CKey CVCCcoinSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CVCCcoinSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CVCCcoinSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CVCCcoinSecret::SetString(const std::string& strSecret) {
