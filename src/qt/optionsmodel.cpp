...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeVCCAmount"))
        settings.setValue("nAnonymizeVCCAmount", 1000);
    nAnonymizeVCCAmount = settings.value("nAnonymizeVCCAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeVCCAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeVCCAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeVCCAmount:
            return QVariant(nAnonymizeVCCAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeVCCAmount:
            nAnonymizeVCCAmount = value.toInt();
            settings.setValue("nAnonymizeVCCAmount", nAnonymizeVCCAmount);
            emit AnonymizeVCCAmountChanged(nAnonymizeVCCAmount);
            break;
        default:
            break;
        }
