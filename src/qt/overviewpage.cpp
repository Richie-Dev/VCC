...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeVCCAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeVCCAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeVCCAmount = strAnonymizeVCCAmount.remove(strAnonymizeVCCAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeVCCAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeVCCAmount*COIN) nMaxToAnonymize = nAnonymizeVCCAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeVCCAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeVCCAmount));
        strAnonymizeVCCAmount = strAnonymizeVCCAmount.remove(strAnonymizeVCCAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeVCCAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
