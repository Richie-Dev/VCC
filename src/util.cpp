...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\VCC
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\VCC
    // Mac: ~/Library/Application Support/VCC
    // Unix: ~/.vcc
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "VCC";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "vcc.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "vccd.pid"));
